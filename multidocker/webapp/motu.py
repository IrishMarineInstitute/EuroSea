...
def motu(USER, PSWD , SERVICE, PRODUCT, OUTPUT_DIRECTORY, OUTPUT_FILENAME, 
         xmin, xmax, ymin, ymax, idate, edate, variable, tipo, zmin=None, zmax=None):
    ...
    if ( zmin is not None ) and ( zmax is not None ):
        ...
        script_template = ('python -m motuclient '
                       ...
                       f'--latitude-min {ymin} --latitude-max {ymax} '
                       f'--date-min "{idate}" --date-max "{edate}" ' 
                       f'--depth-min {zmin} --depth-max {zmax} '
                       f'--variable {variable} '
                       '--out-dir <OUTPUT_DIRECTORY> '
                       '--out-name <OUTPUT_FILENAME> '
                       '--user <USERNAME> --pwd <PASSWORD>'
