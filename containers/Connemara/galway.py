...
def reader():
    ''' Read Connemara model sea level time series at the 
        indicated site (LAT, LON) '''
        
    url = 'http://milas.marine.ie/thredds/dodsC/connemara_native/connemara_native_aggregate.nc'
    ...
    return x, y, time, zeta, surface_temperature, surface_salinity 
...
def main():
    ...
    try:
        ...
        ''' Get distances and bearings to true pier locations '''
        distances, bearings = config.get('dist').split(','), config.get('angles').split(',')

        ''' Read Connemara model '''
        logger.info(f'{now()} Reading from Galway Bay THREDDS...')
        x, y, time, zeta, surf_tem, surf_sal = reader()

        for name, longitude, latitude, distance, bearing in zip(names, longitudes, latitudes, distances, bearings):
            ...
            values = dict(tidewet=SEA_LEVEL, time=webtime,
                names=nicename, lon=lonstr, lat=latstr, distance=distance, bearing=bearing,
