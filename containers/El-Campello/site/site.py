...
def SITE(date=datetime.now()):
    ...
    try:
        ...
        # 15-meter depth currents
        z15_series, z15_fig = windrose(sub, level='15m')
        # Winds 
        wind_series, wind_fig = windrose(sub, level='wind', vartype='wind')
        # Wave period
        wave_series, wave_fig = windrose(sub, level='wave', vartype='wave')

        ''' Add wind series for time series plot (subset every 3 hours) '''
        wind3h = {
                'time-3h': sub.index[0::18],
                's-wind-3h': sub.get('s-wind')[0::18],
                'd-wind-3h': sub.get('d-wind')[0::18]
                }
        wind3h['u-wind-3h'], wind3h['v-wind-3h'] = get_uv(wind3h.get('s-wind-3h'), wind3h.get('d-wind-3h'), 'FROM')

        # Get time zone
        timezone = config['timezone']
                
        logger.info(f'{now()} SEND OUTPUT...')
        send_output(sub, clim, timezone,
